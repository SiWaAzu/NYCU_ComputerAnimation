...
void Cloth::initializeSpring() {
  ...
  float bendLenght = (_particles.position(0) - _particles.position(2)).norm();
  for (int i = 0; i < particlesPerEdge; ++i) {
    for (int j = 0; j < particlesPerEdge - 1; ++j) {
      ...
      if (j < particlesPerEdge - 2) {
        _springs.emplace_back(index, index + 2, bendLenght, Spring::Type::BEND);
      }
    }
  }
  ...
  for (int i = 0; i < particlesPerEdge - 1; i++) {
    for (int j = 0; j < particlesPerEdge; j++) {
      _springs.emplace_back(i * particlesPerEdge + j, (i + 1) * particlesPerEdge + j, structrualLength,Spring::Type::STRUCTURAL);
      if (i < particlesPerEdge - 2) {
        _springs.emplace_back(i * particlesPerEdge + j, (i + 2) * particlesPerEdge + j, bendLenght, Spring::Type::BEND);
      }
    }
  }
  ...
  //shear spring
  float shearLength = (_particles.position(0) - _particles.position(particlesPerEdge)).norm();
  for (int i = 0; i < particlesPerEdge-1; ++i) {
    for (int j = 0; j < particlesPerEdge; ++j) {
      if (j == 0) {
        //Ãä¬É±¡ªp
        _springs.emplace_back(i * particlesPerEdge + j, (i + 1) * particlesPerEdge + 1+j,
                              shearLength, Spring::Type ::SHEAR);
      } else if (j == particlesPerEdge - 1) {
        _springs.emplace_back(i * particlesPerEdge + j, (i + 1) * particlesPerEdge - 1+j, shearLength,Spring::Type ::SHEAR);
      } else {
        _springs.emplace_back(i * particlesPerEdge + j, (i + 1) * particlesPerEdge + 1+j, shearLength,Spring::Type ::SHEAR);
        _springs.emplace_back(i * particlesPerEdge + j, (i + 1) * particlesPerEdge - 1+j, shearLength,Spring::Type ::SHEAR);
      }
...
void Cloth::computeSpringForce() {
  ...
  for (const auto& spring : _springs) {
    // Write code here!
    int aIdx = spring.startParticleIndex();
    int bIdx = spring.endParticleIndex();
    ...
    auto vDotL = (_particles.velocity(aIdx) - _particles.velocity(bIdx))
                     .dot(_particles.position(aIdx) - _particles.position(bIdx)) /
                 distAB;
